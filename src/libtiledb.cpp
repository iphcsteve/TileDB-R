...
const char* _tiledb_datatype_to_string(tiledb_datatype_t dtype) {
  switch (dtype) {
    ...
    case TILEDB_ANY:
      return "ANY";
    default:
      Rcpp::stop("unknown tiledb_datatype_t");
  }
}
...
const char* _tiledb_filter_to_string(tiledb_filter_type_t filter) {
  switch(filter) {
    ...
    default: {
      Rcpp::stop("unknown tiledb_filter_t");
    }
  }
}
...
const char* _tiledb_filter_option_to_string(tiledb_filter_option_t filter_option) {
  ...
  default: {
    Rcpp::stop("unknown tiledb_filter_option_t");
  }
  }
}
...
std::string _tiledb_query_type_to_string(tiledb_query_type_t qtype) {
  switch (qtype) {
    ...
    case TILEDB_WRITE:
      return "WRITE";
    default:
      Rcpp::stop("unknown tiledb_query_type_t");
  }
}
...
XPtr<tiledb::Dimension> libtiledb_dim(XPtr<tiledb::Context> ctx,
  ...
  try {
    if (_type == TILEDB_INT32) {
      ...
      Rcpp::stop("Unsupported tiledb type (id): %d this should not happen!", _type);
    }
...
NumericVector dim_domain_subarray(NumericVector domain, NumericVector subscript) {
  ...
  for (R_xlen_t i = 1; i < subscript_length; i++) {
    ...
    if (high < domain_lb || high > domain_ub) {
      Rcpp::stop("subscript out of domain bounds: (at index: [%d] %ld < %ld", i, high, domain_lb);
    }
...
List libtiledb_array_nonempty_domain(XPtr<tiledb::Array> array) {
  ...
  if (domain.type() == TILEDB_INT32) {
    ...
    Rcpp::stop("Invalid tiledb_schema domain type");
  }
...
// NB we read val, return a string _and_ also modify offsets _in place_
// [[Rcpp::export]]
std::string libtiledb_query_set_buffer_var_char_helper(SEXP val, NumericVector origoffset) {
  ...
  // now memcpy the uint64_t vector into the double vector we use for 'transport'
  ...
  if (TYPEOF(val) == VECSXP) {
    ...
    if (TYPEOF(nval) == VECSXP) {
      ...
      if (TYPEOF(nnval) == INTSXP) {
        ...
        res = Rcpp::wrap(pairres.first);
        std::memcpy(&origoffset[0], pairres.second.data(), pairres.second.size()*sizeof(uint64_t));
      } else if (TYPEOF(nnval) == STRSXP) {
        std::string str = libtiledb_query_set_buffer_var_char_helper(val, origoffset);
        res = Rcpp::wrap(str);
      } else {
